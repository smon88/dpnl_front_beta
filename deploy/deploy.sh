...
#
# Uso:
#   ./deploy.sh panel     - Desplegar solo el panel Laravel
#   ./deploy.sh backend   - Desplegar solo el backend Node
#   ./deploy.sh all       - Desplegar ambos
...
check_requirements() {
    ...
    if ! command -v pm2 &> /dev/null; then
        log_error "PM2 no esta instalado"
    fi
...
# ============================================
# Deploy Panel Laravel
# ============================================
deploy_panel() {
    log_info "=========================================="
    log_info "Desplegando Panel Laravel..."
    log_info "=========================================="

    create_backup "panel" "$PANEL_DIR"
    ...
    log_success "Panel desplegado exitosamente"
}
...
deploy_backend() {
    log_info "=========================================="
    log_info "Desplegando Backend Node..."
    log_info "=========================================="
    ...
    # Ejecutar migraciones si existen (Prisma, Sequelize, etc)
    ...
    # Reiniciar con PM2
    log_step "Reiniciando aplicacion con PM2..."
    ...
    sleep 3
    if pm2 describe devil-backend | grep -q "online"; then
        log_success "Backend desplegado y corriendo"
    else
        log_error "El backend no inicio correctamente. Revisa los logs: pm2 logs devil-backend"
    fi
}
...
rollback() {
    ...
    if [ -z "$app" ]; then
        echo "Uso: ./deploy.sh rollback [panel|backend]"
        exit 1
    fi
    ...
    case $app in
        panel)
            backup_path="$BACKUP_DIR/panel"
            restore_dir="$PANEL_DIR"
            ;;
        backend)
            backup_path="$BACKUP_DIR/backend"
            restore_dir="$BACKEND_DIR"
            ;;
        *)
...
health_check() {
    ...
    if systemctl is-active --quiet postgresql; then
        ...
        log_warning "PostgreSQL: Inactivo"
    fi
    ...
    # PM2 / Backend
    if pm2 describe devil-backend 2>/dev/null | grep -q "online"; then
        log_success "Backend Node: Activo"
    else
        log_warning "Backend Node: Inactivo o no configurado"
    fi
    ...
    if curl -sf -o /dev/null "http://localhost/"; then
        log_success "Panel responde correctamente"
    else
        log_warning "Panel no responde en localhost"
    fi
    ...
    echo ""
    log_info "Para ver logs de PM2: pm2 logs devil-backend"
    log_info "Para ver logs de Laravel: tail -f $PANEL_DIR/storage/logs/laravel.log"
}
